...
import { authState } from "../firebase/auth";
  ...
  export let data;
  let authStatus = data.auth.authState;
  ...
  <main class="flex justify-center items-center h-full w-full ">
    <div class="flex flex-col p-4  backdrop-blur-lg w-1/3 h-1/3 min-w-80 justify-between shadow-lg">
      <div class="flex flex-col space-y-4">
        <input class="px-2 box-border rounded-md h-8 border-2 border-vintageBeige border-solid" type="email" placeholder="Email" bind:value={email} />
        <input class="px-2 box-border rounded-md h-8 border-2 border-vintageBeige border-solid" type="password" placeholder="Password" bind:value={password} />
      </div>
      <div class="flex space-x-4 justify-end">
        ...
        <button class="p-2 rounded-md hover:text-vintageBeige active:opacity-80 text-white" on:click={handleSignIn} disabled={logginIn}>LogIn</button>
      </div>
    </div>
  </main>
